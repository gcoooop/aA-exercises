...
def cats_like_johnson
  ...
  execute(<<-SQL)
    ...
    SELECT
      other_cats.name
    FROM
      cats AS johnson_cats
    JOIN
      cats AS other_cats ON other_cats.breed = johnson_cats.breed
    WHERE
      johnson_cats.name = 'Johnson'
      AND
      johnson_cats.color = 'Lavender'
    ORDER BY
      other_cats.name;
  SQL
end
