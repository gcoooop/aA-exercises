...
def price_like_shiny_mouse
  ...
  execute(<<-SQL) 
    SELECT
      price_toys.name, price_toys.price
    FROM  
      toys AS shiny_toys
    JOIN
      toys AS price_toys ON shiny_toys.price = price_toys.price
    WHERE
      shiny_toys.name = 'Shiny Mouse'
      AND
      price_toys.name != 'Shiny Mouse'
    ORDER BY
      price_toys.name;
  SQL
end
...
def just_like_orange
  ...
  execute(<<-SQL)
    SELECT
      other_cats.name, other_cats.breed
    FROM
      cats AS orange_cats
    JOIN
      cats AS other_cats ON orange_cats.breed = other_cats.breed
    WHERE
      orange_cats.name = 'Orange'
      AND
      other_cats.name != 'Orange'
    ORDER BY
      name;
  SQL
end
...
def toys_that_jet_owns
  ...
  execute(<<-SQL)
    SELECT
      DISTINCT(other_cats.name), toys.name
    FROM
      cats AS jet_cats
    JOIN
      cattoys ON cattoys.cat_id = jet_cats.id
    JOIN
      toys ON toys.id = cattoys.toy_id
    JOIN
      cattoys AS other_cattoys ON other_cattoys.toy_id = toys.id
    JOIN
